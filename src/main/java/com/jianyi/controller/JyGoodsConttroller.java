...
//----------------------------------------------------------------------------------------------------------------------
    ...
    {
        //构造搜索条件
        QueryBuilder builder = QueryBuilders.existsQuery("goods_name");

        SearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(builder).build();

        List<JyGoods> jyGoods = elasticsearchTemplate.queryForList(searchQuery,JyGoods.class);
        for (JyGoods jyGoods1 : jyGoods)
        {
            System.out.println(jyGoods);
        }
        return "success";
    }
}